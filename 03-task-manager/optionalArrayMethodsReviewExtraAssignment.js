...
const lastNames = names.map((person) => {
  const splitedNames = person.split(" ");
  return splitedNames[splitedNames.length - 1]
})

const filteredNames = names.filter((person) => {
  return person.split(" ").length === 2;
})

const titleCaseNames = names.map((person) => {
  const splittedNames = person.split(" ");
  ...
  splittedNames.forEach(name => {
    if(name.length > 0) {
      ...
      formattedNameList.push(formattedName)  
    }
  })
...
const lastChallengeNames = names
                            .filter( (person) => {
                              return person.split(" ").length === 2;
                            })
                            .map((person) => {
...
lastChallengeNames.forEach(person => {
  console.log(`Would you like to sign up, ${person}?`)
})
